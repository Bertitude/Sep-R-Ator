...
class SepReformerProcessor:
    ...
    def _write_cpu_wrapper(self) -> None:
        ...
        wrapper.write_text(
            ...
            "\n"
            "Core idea: subclass torch.device (not replace it with a function).\n"
            "A subclass preserves isinstance() semantics that PyTorch's own\n"
            "serialisation code relies on, while still redirecting every\n"
            "torch.device('cuda', ...) construction to cpu — meaning self.device\n"
            "is already cpu everywhere in SepReformer, so tensor.to(self.device),\n"
            "model.to(self.device), and torch.load(map_location=self.device)\n"
            'all just work without needing Tensor.to patching."""\n'
            "import runpy, sys, torch as _torch\n"
            "\n"
            "if not _torch.cuda.is_available():\n"
            "    _OrigDevice = _torch.device\n"
            "\n"
            "    class _CpuDevice(_OrigDevice):\n"
            "        def __new__(cls, type_or_device='cpu', index=None):\n"
            "            if isinstance(type_or_device, str) and type_or_device.startswith('cuda'):\n"
            "                return _OrigDevice.__new__(cls, 'cpu')\n"
            "            if index is not None:\n"
            "                return _OrigDevice.__new__(cls, type_or_device, index)\n"
            "            return _OrigDevice.__new__(cls, type_or_device)\n"
            "\n"
            "    _torch.device = _CpuDevice\n"
            "\n"
            "    # Belt-and-suspenders: patch torch.load in case map_location\n"
            "    # arrives as a pre-existing _OrigDevice('cuda:0') object.\n"
            "    _orig_load = _torch.load\n"
            "    def _cpu_load(f, map_location=None, **kwargs):\n"
            "        if map_location is None:\n"
            "            map_location = 'cpu'\n"
            "        elif isinstance(map_location, _OrigDevice) and map_location.type == 'cuda':\n"
            "            map_location = 'cpu'\n"
            ...
            "\n"
            "    # Belt-and-suspenders: redirect nn.Module.to(cuda) → cpu.\n"
            "    import torch.nn as _nn\n"
            "    _orig_module_to = _nn.Module.to\n"
            "    def _cpu_module_to(self, *args, **kwargs):\n"
            "        args = list(args)\n"
            "        if args:\n"
            "            if isinstance(args[0], _OrigDevice) and args[0].type == 'cuda':\n"
            "                args[0] = 'cpu'\n"
            "            elif isinstance(args[0], str) and args[0].startswith('cuda'):\n"
            "                args[0] = 'cpu'\n"
            "        if 'device' in kwargs:\n"
            "            d = kwargs['device']\n"
            "            if isinstance(d, _OrigDevice) and d.type == 'cuda':\n"
            "                kwargs['device'] = 'cpu'\n"
